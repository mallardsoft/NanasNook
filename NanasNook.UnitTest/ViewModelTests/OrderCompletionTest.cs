...
namespace NanasNook.UnitTest.ViewModelTests
{
    ...
    public class OrderCompletionTest
    {
        ...
        private void PlaceAndPullOrder()
        {
        ...
        [TestMethod]
        public void CompleteAnOrder_DisappearsFromOrdersInProgress()
        {
            CompleteOrder();

            Assert.IsFalse(_kitchenViewModel.OrdersInProgress.Any());
        }

        [TestMethod]
        public void CompleteAnOrder_AppearsInOrdersCompleted()
        {
            CompleteOrder();

            k.CompletedSummaryViewModel pull = _kitchenViewModel.OrdersCompleted.Single();
            Assert.AreEqual("Allen", pull.City);
            Assert.AreEqual(new DateTime(2010, 8, 19, 14, 0, 0), pull.ExpectedDeliveryDate);
            Assert.AreEqual("Full Sheet 3\"", pull.Size);
        }

        [TestMethod]
        public void CompleteAnOrder_OrderCompletedIsSelected()
        {
            CompleteOrder();

            Assert.AreEqual(_kitchenViewModel.OrdersCompleted.Single(), _kitchenViewModel.SelectedCompleteOrder);
        }

        [TestMethod]
        public void CompleteAnOrder_StatusIsCompleted()
        {
            CompleteOrder();
            Synchronize();

            Assert.AreEqual("Completed", _frontOfficeViewModel.PendingOrders.Single().Status);
        }
